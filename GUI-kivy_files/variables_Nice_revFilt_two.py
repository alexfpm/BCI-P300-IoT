...
# OPTIONS:
#
#   • mode = 1     - Reads only 1 pair of files (DataFromArduino and subject-0)
#   • mode = 2     - Reads all the files from path
#   • mode = 3     - Optimization mode for a group of files in a folder
#   • mode = 4     - Optimization mode for a single file in the Path
#
#   • test: Type of test to which the data corresponds (1 or 2)
...
def erps_calculation(arduinoData, opSesameData, datesArduino):
    ...
    # #OPTION 1: PUTTY
    # f = open('DataFromArduino.txt', 'r')
    # medicionesRAW = f.read()
    # f.close()
    ...
    #-----------------------------------------------------
    #TEST2: FILTERING FFT FROM 0.5 TO 30 Hz
    ...
    #ch2_Filtered = filtered_CH2*1000
    ...
    for x in range(0, len(ch2_Filtered)):
        ...
        if val > 0.3:
            ...
            ch2_Filtered[cont-10 : cont+10] = ch2_Filtered[cont-10 : cont+10]/10
    ...
    # ch2_Nice = np.empty(shape = (15000))
    
    # ch2_Nice[125 : len(ch2_Nice)] = distancesEEG2
    
    # ch2_Nice[0 : 124] = np.mean(distancesEEG2)
    ...
    #CH2
    samplerate = 250.
    timeLine = np.arange(y2.shape[0]) / samplerate
    ...
    return erps, flagInd, samplerate, negative_shape, y2
    #-----------------------------------------------------------------
...
if mode == 1 or mode == 4:
    ...
    erps_ch2, flagInd, samplerate, neg_shape, ch2_Cz = erps_calculation(mediciones[0], timesOpSesame[0], timesArduino[0])
    #------------------------------------------
...
#-----------------------------------------------------------------
    ...
    for file in os.listdir():
        ...
        if file.endswith(".txt"):
            ...
            #--------------------------------------------------
            #Saving the name of the objectives
            names_files.append(file[len(file)-7 : len(file)-4])
            #--------------------------------------------------
    ...
    # Multidimensional ERPs, to send the ERPs of files
    erpsM = []
    
    
    # Multidimensional negative-shape, to send the results for files
    neg_shape_M = []
    
    
    # Call the ERP function
    for x in range(len(mediciones)):
        
        erps, flagInd, samplerate, neg_shape = erps_calculation(mediciones[x], timesOpSesame[x], timesArduino[x])
        
        
        erpsM.append(erps)
        
        neg_shape_M.append(neg_shape)
