...
def erps_calculation(arduinoData, opSesameData, datesArduino):
    ...
    ch3_Nice = np.empty(shape = (15000))
    
    ch3_Nice[125 : len(ch3_Nice)] = distancesEEG2
    
    ch3_Nice[0 : 124] = np.mean(distancesEEG2)
    ...
    labels2 = np.zeros(len(ch3_Nice))
    
    ch3_Nice = ch3_Nice*1000
    
    
    #CH2
    samplerate = 250.
    timeLine = np.arange(ch3_Nice.shape[0]) / samplerate
    ...
    for w in range(len(flagInd)):
        ...
        for x in range(flagInd2[w]):
            ...
            trialsFlags[w].append(ch3_Nice[aux1[x] : aux1[x]+snippet])
    #-----------------------------------------------------------------    
    ...
    return erps, flagInd, samplerate, negative_shape, ch3_Nice
    #-----------------------------------------------------------------
