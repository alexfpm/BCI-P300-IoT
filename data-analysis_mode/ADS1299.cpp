...
void ADS1299::updateChannelData(){
  ...
  //IMPRIMIR RESULTADOS NUMÉRICOS (MEDICIONES DE LOS CANALES) EN LA SERIAL CONSOLE
  //Comentarear la sección del segundo if para verificar datos del Print sin filtros/tratamientos
  if(verbose){
    Serial.print(stat_1); Serial.print(", ");
    
    for(int i=0; i<8; i++){

      EEGData[i]=channelData[i];
      Serial.print(channelData[i]);
      
      /*if (((channelData[i]*(2.5))/((2^24)-1)) > 5000 || ((channelData[i]*(2.5))/((2^24)-1)) < -5000){
        
        Serial.print(0);
        
      } else {
          EEGData[i]=(channelData[i]*(0.37))/((2^24)-1);
        ...
        */
      //Descomentarear siguiente línea para verificar datos que se imprimen sin ningún tipo de filtro/tratamiento
      //Serial.print((channelData[i]*(2.5))/((2^24)-1));
      if(i<8){Serial.print(", ");}
    }
    Serial.println();
  }
